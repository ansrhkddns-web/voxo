...
export function AdminCategoriesClient({
    ...
    const [categories, setCategories] = useState<CategoryManagementItem[]>(initialCategories);
    ...
    useEffect(() => {
        ...
        toast.error(
            isKorean
                ? '카테고리 데이터를 일부 불러오지 못했습니다.'
                : 'Some category data could not be loaded.'
        );
        setLoading(true);
        void loadCategories();
    }, [initialLoadFailed, isKorean, loadCategories]);
                    ...
                    <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
                        <div className="space-y-2">
                            ...
                            <p className="max-w-2xl text-sm text-gray-500">
                                {isKorean
                                    ? '사용 중인 카테고리와 비어 있는 카테고리를 한 번에 확인하고 안전하게 관리합니다.'
                        ...
                        <form
                            onSubmit={handleAdd}
                            className="grid gap-3 sm:grid-cols-[minmax(0,280px)_180px_auto]"
                        >
                            <input
                            ...
                            <div className="flex flex-wrap gap-2">
                                {[
                                    ...
                                    <button
                                        ...
                                        type="button"
                                        onClick={() =>
                                            setFilterMode(item.key as 'all' | 'in-use' | 'unused')
                                        }
