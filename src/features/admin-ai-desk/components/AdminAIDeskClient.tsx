...
import { getAiPromptManagerData, type AIPromptManagerData } from '@/app/actions/aiPromptActions';
...
interface AdminAIDeskClientProps {
    initialData: AIPromptManagerData | null;
    initialLoadFailed?: boolean;
}

export default function AdminAIDeskClient({
    initialData,
    initialLoadFailed = false,
}: AdminAIDeskClientProps) {
    ...
    const [formData, setFormData] = useState<AIDeskFormState>(() => ({
        ...DEFAULT_AI_DESK_FORM,
        categoryId: initialCategories[0]?.id || DEFAULT_AI_DESK_FORM.categoryId,
    }));
    ...
    const [promptConfig, setPromptConfig] = useState<AIPromptManagerConfig>(
        buildResolvedPromptManagerConfig(initialData?.managerConfigRaw ?? null, initialCategories)
    );
    ...
    useEffect(() => {
        ...
        setPromptConfig(
            buildResolvedPromptManagerConfig(initialData?.managerConfigRaw ?? null, nextCategories)
        );
        setFormData((prev) => ({
            ...prev,
            categoryId:
                prev.categoryId && nextCategories.some((category) => category.id === prev.categoryId)
                    ? prev.categoryId
                    : nextCategories[0]?.id || '',
        }));
    }, [initialData]);
    ...
    useEffect(() => {
        ...
        const fetchDeskData = async () => {
            try {
                ...
                setCategories(nextCategories);
                setPromptConfig(
                    buildResolvedPromptManagerConfig(data.managerConfigRaw, nextCategories)
                );
                setFormData((prev) => ({
                    ...prev,
                    categoryId:
                        prev.categoryId ||
                        nextCategories[0]?.id ||
                        DEFAULT_AI_DESK_FORM.categoryId,
                }));
            } catch (fetchError) {
