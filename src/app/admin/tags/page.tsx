...
export default function TagsPage() {
    ...
    const [tags, setTags] = useState<TagManagementItem[]>([]);
    ...
    const handleQuickMenuToggle = async (tag: TagManagementItem) => {
        try {
            ...
            toast.error(isKorean ? '메뉴 노출 변경에 실패했습니다.' : 'Failed to update menu visibility.');
        }
    };

    const handleDelete = async (tag: TagManagementItem) => {
        if (!confirm(isKorean ? `"${tag.name}" 태그를 삭제하시겠습니까?` : `Delete "${tag.name}"?`)) {
            return;
        }
    ...
    const filteredTags = useMemo(() => {
        return tags
            ...
            .sort((left, right) => {
                ...
                return (left.menu_order || 0) - (right.menu_order || 0) || left.name.localeCompare(right.name);
            });
    }, [filterMode, searchQuery, sortMode, tags]);

    useEffect(() => {
        setSelectedTagIds((prev) => prev.filter((id) => filteredTags.some((tag) => tag.id === id)));
    }, [filteredTags]);
    ...
    const allFilteredSelected = filteredTags.length > 0 && filteredTags.every((tag) => selectedTagIds.includes(tag.id));
    ...
    const handleBulkMenuVisibility = async (showInMenu: boolean) => {
        try {
            ...
            toast.error(isKorean ? '일괄 메뉴 노출 변경에 실패했습니다.' : 'Failed to update selected tags.');
        }
    };
    ...
    const handleBulkDelete = async () => {
        if (
            !confirm(
                isKorean
                    ? `선택한 ${selectedCount}개 태그를 정리하시겠습니까? 사용 중인 태그는 자동으로 보호됩니다.`
        ...
        try {
            ...
            toast.error(isKorean ? '일괄 삭제에 실패했습니다.' : 'Failed to delete selected tags.');
        }
    };
                    ...
                    <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
                        <div className="space-y-2">
                            ...
                            <p className="max-w-2xl text-sm text-gray-500">
                                {isKorean
                                    ? '메뉴 노출 태그와 실제 사용 중인 태그를 함께 확인하고 빠르게 관리하세요.'
                                    : 'Review menu tags and in-use tags together and manage them quickly.'}
                            </p>
                        </div>

                        <form onSubmit={handleAdd} className="grid gap-3 lg:grid-cols-[auto_minmax(0,220px)_120px_220px_auto] lg:items-center">
                            ...
                            <input
                                ...
                                onChange={(event) => setNewTagMenuOrder(parseInt(event.target.value, 10) || 0)}
                            />
                        ...
                        <section className="flex flex-col gap-4 border border-white/10 bg-white/[0.02] p-5 lg:flex-row lg:items-center lg:justify-between">
                            <div className="relative w-full max-w-md">
                                ...
                                <input
                                    ...
                                    placeholder={isKorean ? '태그 이름 또는 슬러그 검색' : 'Search by name or slug'}
                            ...
                            <div className="flex flex-wrap gap-2">
                                {[
                                    ...
                                    <button
                                        key={item.key}
                                        type="button"
                                        onClick={() => setFilterMode(item.key as 'all' | 'menu' | 'in-use')}
                                ...
                                <select
                                    value={sortMode}
                                    onChange={(event) => setSortMode(event.target.value as 'menu-order' | 'usage' | 'name')}
                                ...
                                >
                                    <option value="menu-order">{isKorean ? '메뉴 순서' : 'Menu order'}</option>
                                    <option value="usage">{isKorean ? '사용량 순' : 'Usage'}</option>
                                    <option value="name">{isKorean ? '이름 순' : 'Name'}</option>
                                </select>
                            </div>
                        </section>
                                        ...
                                        {filteredTags.map((tag) => (
                                            ...
                                            >
                                                <td className="px-6 py-5">
                                                    ...
                                                    >
                                                        {selectedTagIds.includes(tag.id) ? <CheckSquare size={16} className="text-accent-green" /> : <Square size={16} />}
                                                    </button>
                                                </td>
                                                <td className="px-6 py-5">
                                                    {editingId === tag.id ? (
                                                        <input
                                                            ...
                                                            onKeyDown={(event) => {
                                                                if (event.key === 'Enter') void handleUpdate(tag.id);
                                                                if (event.key === 'Escape') {
                                                                    setEditingId(null);
                                                                    setEditingName('');
                                                                }
                                                ...
                                                <td className="px-6 py-5 font-mono text-[11px] text-gray-500">
                                                    {editingId === tag.id ? buildTagSlug(editingName) || 'tag-slug' : tag.slug}
                                                </td>
                                                ...
                                                <td className="px-6 py-5">
                                                    {editingId === tag.id ? (
                                                        ...
                                                        <div className="flex flex-wrap items-center gap-2">
                                                            <button
                                                                type="button"
                                                                onClick={() => void handleQuickMenuToggle(tag)}
                                                            ...
                                                            >
                                                                {tag.show_in_menu ? <CheckSquare size={12} /> : <Square size={12} />}
                                                                <span>{tag.show_in_menu ? (isKorean ? '노출 중' : 'Visible') : (isKorean ? '숨김' : 'Hidden')}</span>
                                                            </button>
                                                            <Link
                                                ...
                                                <td className="px-6 py-5">
                                                    {editingId === tag.id ? (
                                                        <input
                                                            ...
                                                            onChange={(event) => setEditingMenuOrder(parseInt(event.target.value, 10) || 0)}
