...
import type { AdminSecurityState, AdminSettingsState, AdminSettingsTabId } from '@/features/admin-settings/types';
...
export default function AdminSettingsPage() {
    ...
    const [security, setSecurity] = useState<AdminSecurityState>(DEFAULT_ADMIN_SECURITY);
    ...
    useEffect(() => {
        const loadSettings = async () => {
            try {
                ...
                if (localSettings) {
                    setSettings((prev) => ({ ...prev, ...(JSON.parse(localSettings) as Partial<AdminSettingsState>) }));
                }

                const values = await Promise.all(adminSettingLoadOrder.map((key) => getSetting(key)));
                ...
                setSecurity((prev) => ({
                    ...prev,
                    adminEmail: values[14] || DEFAULT_ADMIN_CREDENTIAL.email,
                }));
            } catch (error) {
    ...
    const updateSecurity = <K extends keyof AdminSecurityState>(key: K, value: AdminSecurityState[K]) => {
        ...
        <div className="flex min-h-screen bg-black text-white selection:bg-accent-green/30">
            ...
            <main className="flex-1 overflow-y-auto">
                <header className="sticky top-0 z-50 flex h-20 items-center justify-between border-b border-white/5 bg-black/80 px-12 backdrop-blur-xl">
                    <h1 className="font-display text-[10px] uppercase tracking-[0.4em] text-gray-400">{copy.title}</h1>
                    <button
                        onClick={handleSave}
                ...
                <div className="mx-auto max-w-6xl p-12">
                    <p className="mb-12 max-w-3xl text-sm leading-relaxed text-gray-500">{copy.subtitle}</p>
                    ...
                    {loading ? (
                        <div className="flex h-64 flex-col items-center justify-center gap-4 border border-white/5 bg-white/[0.01]">
                            ...
                            <p className="font-display text-[10px] uppercase tracking-[0.3em] text-gray-600">{copy.loading}</p>
                        </div>
                    ) : (
